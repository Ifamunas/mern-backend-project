...
export const getUsers = async (page = 1, limit = 5, sort: string, search = '') => {
  ...
  const notAdmin = true
  const filterUsers = searchItems({ search })
  ...
  let html: string;
    ...
    case 'activate':
      ...
      html = `<h1> Hello ${name} </h1> 
        ...
        <a href="http://localhost:${dev.app.frontendport}/users/activate/${token} "> activate your account </a></p>`;
      break;
    ...
    case 'reset-password':
      ...
      html = `<h1> Hello ${name} </h1> 
        <p>Please click here to:
        <a href="http://localhost:${dev.app.frontendport}/users/reset-password/${token} "> reset your password</a></p>`;
      break;
